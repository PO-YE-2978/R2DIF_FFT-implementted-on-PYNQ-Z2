...
* AXI 介面分成 Master (主動發起 transaction) 和 Slave (被動接收) 端，其中 : 
...
## 8.2 IP Package 流程 :
   1. 第一步先建立好 FFT Project (板子選 PYNQ Z2)，接著在 ```Tools```中點選```create and package new IP```後，再選```Create AXI4 Peripheral```。
   2. Defalut name 為 myip_v1_0.v 和 myip_v1_0_S00_AXI.v，此處我們將名稱改為 my_fft_top_ip，也就是 my_fft_top_ip_v1_0.v 和 my_fft_top_ip_v1_0_S00_AXI.v
   ...
   3. Add Interface 中確認 Interface type 為 Lite (代表 AXI4-Lite)，Mode 是 Slave (FFT Core 為 slave)，Data Width 和 Number of Register 保留 32 和 4 即可點選 Next。
   4. 最後一步 Create Peripheral 中確認資訊，同時選擇 Edit IP 後即可創建。
   5. 創建完後會產生新的 Vivado Project，其中包含第二點提到的兩個 ip packaging code。在這兩個 code 的 I/O 中補上 BRAM 的 port (詳見 ```code```)
...
## 8.3 Chapter Review
本章介紹了 AXI Interface 的概念以及如何將我們的 code package 成 IP。簡單來說，AXI interface 就是要讓 PS 端跟 PL 端能夠溝通的橋樑。為此，我們需要將 project package 成 IP 並連結 AXI、Processor 等，全部包起來產生 .bit file 後才能運作。下一章將介紹如何將這些 block 接起來。
