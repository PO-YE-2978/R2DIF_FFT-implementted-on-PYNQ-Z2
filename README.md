...
- 開發環境：Xilinx Vivado（建立/封裝 IP、產生 bitstream）、PYNQ（在板上執行 Notebook 驗證）
...
4. 部署到 PYNQ：
   - 從 Vivado 取得 `.bit` 與 `.hwh` 檔案，並放到 PYNQ 板的 overlay 目錄下，檔名需一致（例如 `r2dif_fft.bit` 與 `r2dif_fft.hwh`）。
   - 在 PYNQ 上啟動 Notebook，載入 Overlay 並透過 MMIO / 驅動控制 start 與讀取結果（請參閱 Chapter 10 的範例）。
5. 模擬測試：
   - 使用 `tb_fft_top.v` 在模擬器中驗證資料流與數值正確性，並與 software FFT（例如 numpy.fft）比對。
...
## 測試與驗證建議
- 在模擬器中執行 `tb_fft_top.v` 並比對 reference FFT（numpy.fft）結果。
...
## 建議的後續改進（TODO）
- 提供一份完整 Vivado Tcl 腳本，自動化建立工程、加入 sources、封裝 IP 到 IP repo、產生成品 bitstream
- 在 PYNQ Notebook 裡加入完整的 register map、數值誤差統計與可視化
- 新增 LICENSE（目前 repo 中未見明確授權），建議採用 MIT 或 Apache-2.0
...
---
如果你要我直接把這份 README 內容 commit 回儲存庫，我已經為你更新（commit message: "Update README: expanded project overview, usage instructions, and PYNQ notes").
